...
class MoveGroupPythonInterfaceTutorial(object):
    ...
    def plan_cartesian_path(self, scale=1):
        ...
        wpose.position.z -= scale * 0.007 # First move up (z)
        wpose.position.y += scale * 0.007  # and sideways (y)
        ...
        wpose.position.x -= scale * 0.01  # Third move sideways (y)
        waypoints.append(copy.deepcopy(wpose))        
    ...
    def execute_plan(self, plan):
        ...
        # 参照状態を取得
        ref_state = move_group.get_current_state()

        # 速度スケーリングを適用
        velocity_scaling_factor = 0.5  # 速度を半分にする
        move_group.retime_trajectory(ref_state, plan, velocity_scaling_factor)
        ...
        '''executeメソッドにはset_max_velがないので時間調整で代用。'''
